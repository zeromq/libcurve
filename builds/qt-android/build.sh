...
##
# Make sure sodium is built and copy the prefix

(android_build_verify_so "libsodium.so" &> /dev/null) || {
    # Use a default value assuming the sodium project sits alongside this one
    test -z "$SODIUM_ROOT" && SODIUM_ROOT="$(cd ../../../sodium && pwd)"
    
    if [ ! -d "$SODIUM_ROOT" ]; then
        echo "The SODIUM_ROOT directory does not exist"
        echo "  ${SODIUM_ROOT}"
        exit 1
    fi
    
    (${SODIUM_ROOT}/builds/qt-android/build.sh) || exit 1
    UPSTREAM_PREFIX=${SODIUM_ROOT}/builds/qt-android/prefix/${TOOLCHAIN_NAME}
    cp -r ${UPSTREAM_PREFIX}/* ${ANDROID_BUILD_PREFIX}
}

##
# Build curve from local source

(android_build_verify_so "libcurve.so" "libczmq.so" "libsodium.so" &> /dev/null) || {
    rm -rf "${cache}/curve"
    (cp -r ../.. "${cache}/curve" && cd "${cache}/curve" \
        && make clean && rm configure config.status)
    rm 
    
    export LIBTOOL_EXTRA_LDFLAGS='-avoid-version'
    
    (cd "${cache}/curve" && ./autogen.sh \
...
android_build_verify_so "libcurve.so" "libczmq.so" "libsodium.so"
